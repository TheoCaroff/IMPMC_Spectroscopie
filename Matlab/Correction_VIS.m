...
REFERENCE = 0; % mettre 0 pour ne pas prendre en compte les colonnes référence dans les mesures.
BACKGROUND = 1;
...
if REFERENCE
    ...
    ref=avecech(:,2).*corr;             % faisceau ref avec ech * corr
else
    ref=sansech(:,3);
end

tr=avecech(:,3)./ref;               % transmission
...
if BACKGROUND
    if REFERENCE
        writematrix(BK2_HT,Repertoire_arriver+file_100(1:end-4)+".csv", 'Delimiter', ';')
    else
    ...
    if REFERENCE
        writematrix(BK2_HT,Repertoire_arriver+file_100(1:end-4)+"_SANSNOIR.csv", 'Delimiter', ';')
    else
